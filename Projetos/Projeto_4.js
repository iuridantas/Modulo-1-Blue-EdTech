...
while (!missao) {
    ...
    console.log();
    contadorDias++;
    console.log(`É o dia ${contadorDias}º, Após uma longa viagem de 18h nosso major chega na base em Kiev , porém antes de seguir para o local de combate nosso 
combatente precisa-se descansar, comer e hidrata-se pois enfrentou uma longa viagem.`);
    ...
    if (horasdormida1 == `sim` || horasdormida1 == `s`) {
        console.log(
            `Durma bem Diego! Amanhã o dia será longo e boa sorte major! `,
        );
        if (horasdormida == `sim` || horasdormida == `s`) {
            diego.mudasono(0);
        } else if (horasdormida == `nao` || horasdormida == `n`) {
    ...
    if (
        ...
        while (!vitoria) {
            console.log(
                `É o dia ${contadorDias}º, voce acorda e precisa realizar a atividade da base!`,
            );
            console.log();
            console.log(
                `Nossos guerreiros precisam sair o mais rapido possivél! Vá ligando os nossos veiculos para ir adiantando! `,
            );
            console.log();
            ...
            if (base == 5) {
                ...
                console.log(
                    `Precisamos de no minino 4 veiculos já ligado, tente novamente.`,
                );
                base = 0;
        ...
        console.log(
            `O major Diego não está 100% para ir para guerra, pois algum teor ficou com apenas 10 pontos, porém ele disposto a ir, entretando por não está 100% ele vai está no morro com uma sniper dando suporte a tropa. Boa sorte major! `,
        );
        console.log();
        ...
        contadorDias++;
        console.log(
            `É o dia ${contadorDias}º e nossos guerreiros estão se locomovendo até o local, entretanto temos tropas inimigas durante o caminho acertem eles para naõ temos percas de tropas`,
        );
        console.log();
        console.log(
            `Para acertar o tiro você precisa adivinhar qual o numero pensado pelo computador.`,
        );
        console.log(
            `Se não acertar em 5 tentativas você falha a missão e o jogo acaba.`,
        );
        console.log();
        acerto = 0;
        let vitoria1 = false;
        while (!vitoria1) {
            ...
            if (numeropess == numerocomp) {
                console.log();
                console.log(
                    `O numero pensado pelo computador foi ${numerocomp}. Você acertou o número!`,
                );
                acerto++
                vitoria1 = true;
                missao = true;
                ...
                console.log(
                    `O numero pensado pelo computador foi ${numerocomp}. Você errou o número!`,
                );
            acerto--;
            if (acerto <= -5) {
                console.log();
